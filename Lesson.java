...
public class Lesson {
	...
	private int createNewLessonSessionId(Connection dbconn) {
		Statement stmt = null;
		ResultSet answer = null;
		int newId = 1;

		try{
			String query = "SELECT MAX(lessonSessionId) AS maxId FROM LessonSession";
			stmt = dbconn.createStatement();
			answer = stmt.executeQuery(query);
	...
	private int createNewLessonTypeId(Connection dbconn) {
		Statement stmt = null;
		ResultSet answer = null;
		int newId = 1;

		try{
			String query = "SELECT MAX(lessonTypeId) AS maxId FROM LessonType";
			stmt = dbconn.createStatement();
			answer = stmt.executeQuery(query);
